...
@section('content')
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-12">
                <form action="{{url('InterVideo/'.$Intervideo->id)}}" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="_method" value="put">
                    {{csrf_field()}}
                    <div class="card ">
                        <div class="card-header card-header-warning">
                            <h4 class="card-title">{{ __('Modier Intervention Video') }}</h4>
                        ...
                        <div class="card-body ">
                            ...
                            <div class=" mt-2 @if($errors->get('title')) has-danger @endif" >
                                <input  type="text" name="title" class="form-control" id="input-name" value="{{$InterVideo->title}}">
                            </div>
                            ...
                            <select name="Depute" class="form-control bg-dark " id="TypeOfConstruction" >
                                @foreach($deputes as $depute)
                                    <option value="{{$depute->id}}"  @if( $InterVideo->user_id == $depute->id) selected @endif>{{$depute->name}}</option>
                                @endforeach
                            </select>
